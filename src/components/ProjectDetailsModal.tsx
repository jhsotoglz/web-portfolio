...
const ProjectDetailsModal: React.FC<Props> = ({ project, onClose }) => {
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex justify-center items-center p-4">
      <div className="bg-main rounded-2xl max-w-3xl w-full p-6 relative shadow-2xl overflow-y-auto max-h-[90vh]">
        {/* Close Button */}
        <button
          onClick={onClose}
          className="absolute top-2 right-2 text-2xl text-red-500 transition"
        >
          ✕
        </button>
        ...
        {Array.isArray(project.pdfs) && project.pdfs.length > 0 && (
          <div>
            <h3 className="font-semibold mb-2 text-black">Project Documents</h3>
